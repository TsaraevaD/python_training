...
@given(parsers.parse('a new data with {name}, {header} and {footer}'), target_fixture='new_data')
def new_data(name, header, footer):
    return Group(name=name, header=header, footer=footer)


@when('I modify the group from the list')
def modify_group(app, random_group, new_data):
...
def verify_group_modified(db, non_empty_group_list, random_group, app, check_ui, new_data):
    old_groups = non_empty_group_list
    new_groups = db.get_group_list()

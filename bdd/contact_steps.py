...
@given(parsers.parse('a new data with {firstname} and {lastname}'), target_fixture='new_data')
def new_data(firstname, lastname):
    return Contact(firstname=firstname, lastname=lastname)


@when('I modify the contact from the list')
def modify_contact(app, random_contact, new_data):
    app.contact.modify_by_id(random_contact.id, new_data)
